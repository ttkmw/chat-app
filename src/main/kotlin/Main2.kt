            ...
            while (true) {
                ...
                while (selectedKeyIterator.hasNext()) {
                    ...
                    if (selectedKey.isAcceptable) {
                        ...
                        handleNewClient(socketChannel, clients, encoder, pendingData, selector)
...
private fun handleNewClient(socketChannel: SocketChannel, clients: ConcurrentHashMap<SocketAddress, SocketChannel>, encoder: java.nio.charset.CharsetEncoder, pendingData: ConcurrentHashMap<SocketChannel, MutableList<ByteBuffer>>, selector: Selector) {
    if (clients.isEmpty()) {
...
private fun sendData(socketChannel: SocketChannel, message: String, pendingData: ConcurrentHashMap<SocketChannel, MutableList<ByteBuffer>>, selector: Selector, encoder: java.nio.charset.CharsetEncoder) {
    val buffer = encoder.encode(CharBuffer.wrap(message))
    try {
        while (buffer.hasRemaining()) {
            socketChannel.write(buffer)
        }
    } catch (e: Exception) {
        // If there's an exception, add the buffer to pendingData and register OP_WRITE
        pendingData.computeIfAbsent(socketChannel) { mutableListOf() }.add(buffer)
