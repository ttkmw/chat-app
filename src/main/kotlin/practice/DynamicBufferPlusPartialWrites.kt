...
private fun handleNewClient(
    ...
    if (clients.isEmpty()) {
        val message =
            "Hello [${(socketChannel.remoteAddress as InetSocketAddress).hostName}, ${(socketChannel.remoteAddress as InetSocketAddress).port}], There is no one in beyond eyesight network.\n"
...
private fun sendData(
    ...
    try {
        ...
        pendingData.compute(socketChannel) { _, value ->
            (value ?: mutableListOf()).apply { add(buffer) }
        }
